 ...
 */
public class PDFBoxInfoStripper extends PDFTextStripper//PDFStreamEngine 
{

    private static final String thisClassName = PDFBoxInfoStripper.class.getSimpleName().toLowerCase();
    ...
    //    PDFBoxInfoStripper.indent
    //    PDFBoxInfoStripper.drop
    static
    {
    ...
    /**
     * Instantiate a new PDFBoxInfoStripper object. This object will load
     * properties from PDFBoxInfoStripper.properties and will not do
     * anything special to convert the text to a more encoding-specific
     * output.
     *
     * @throws IOException If there is an error loading the properties.
     */
    public PDFBoxInfoStripper() throws IOException
    {
        super( ResourceLoader.loadProperties(
                "./configs/PDFTextStripper.properties", true ) );
    ...
    /**
     * Instantiate a new PDFBoxInfoStripper object.  Loading all of the operator mappings
     ...
     */
    public PDFBoxInfoStripper( Properties props ) throws IOException
    {
        super( props );
        this.outputEncoding = null;
        normalize = new TextNormalize(this.outputEncoding);
    ...
    /**
     * Instantiate a new PDFBoxInfoStripper object. This object will load
     * properties from PDFBoxInfoStripper.properties and will apply
     * encoding-specific conversions to the output text.
     *
     * @param encoding The encoding that the output will be written in.
     * @throws IOException If there is an error reading the properties.
     */
    public PDFBoxInfoStripper( String encoding ) throws IOException
    {
        super( ResourceLoader.loadProperties(
                //"org/apache/pdfbox/resources/PDFBoxInfoStripper.properties", true ));
        		"./configs/PDFBoxInfoStripper.properties", true ));
    ...
    /**
     * @deprecated
     * @see PDFBoxInfoStripper#getText( PDDocument )
    ...
    /**
     * @deprecated
     * @see PDFBoxInfoStripper#writeText( PDDocument, Writer )
    ...
    protected void startDocument(PDDocument pdf) throws IOException
    {
    	...
    	//reset line count and page count
    	curLineNumber = 0;
    	curPageNumber = 0;
    ...
    protected void endPage( PDPage page ) throws IOException
    {
    	...
    	//write pageNumber
    	writeString("[::PAGE::]"+(++curPageNumber)+"\n");
    }
    ...
    public static String getInfoString(List<TextPosition> tps, int lineNumber)
    {
    	String result = "";
    	double fs_avg = 0;
    	double fs_mode = 0;
    	double fs_first = 0;
    	double fs_max = 0;
    	double fs_min = 99999.99;
    	double fs_variance = 0;
    	double indentation = 0;
    	double indentation_avgoffirst4 = 0;
    	int num_words = 0;
    	HashSet<String> fontStyleSet = new HashSet<String>();
    	int numFontStyleSwitches = 0;
    	
    	double first_char_pos_y = 0.0;
    	double avg_pos_y = 0.0;
    	double pos_x_start = 0.0;
    	double pos_x_end = 0.0;
    	
    	int are_all_chars_bold = 1;
    	
    	HashMap<Double, Integer> modeMap = new HashMap<Double, Integer>();
    	
    	int numNonSpaceTPs = 0;
    	String tempCurFontStyle = null;
    	
    	int numFirstCarIndent = 0;
    	indentation_avgoffirst4 = 0.0;
    	...
    	{	
    		...
    		try{
	    		if(!tp.getFont().getBaseFont().toLowerCase().contains("bold"))
	    		{
	    			are_all_chars_bold = 0;
	    		}
    		}catch(java.lang.NullPointerException Ne)
    		{
    			
    		}
    		if(tp.getCharacter() == null)
    		{	num_words++;
    			continue;
    		}
    		
    		if(numNonSpaceTPs == 0)//first character
    		{
    			indentation = tp.getX();
    		}
    		
    		numFirstCarIndent++;
    		if(numFirstCarIndent == 5)
    		{
    			indentation_avgoffirst4 = indentation_avgoffirst4/4.0;
    		}
    		else if(numFirstCarIndent <=4)
    		{
    			indentation_avgoffirst4 += tp.getX();
    		}
    		
    		
    		if(tempCurFontStyle == null)
    		{
    			tempCurFontStyle = tp.getFont().getBaseFont();
    		}
    		else
    		{
    			if(!tempCurFontStyle.equals(tp.getFont().getBaseFont()))
    			{
    				numFontStyleSwitches++;
    				tempCurFontStyle = tp.getFont().getBaseFont();
    			}
    		}
    		
    		numNonSpaceTPs++;
    		double fontsize = tp.getHeight();	//using height as fontsize
    		
    		//collect font style
    		fontStyleSet.add(tp.getFont().getBaseFont());
    		
    		//accumulate sum
    		fs_avg += fontsize;
    		
    		//get first fontsize
    		if(fs_first <= 0.0) fs_first = fontsize;
    		
    		//get max fontsize
    		if(fontsize > fs_max) fs_max = fontsize;
    		
    		//get min fotnsize
    		if(fontsize < fs_min) fs_min = fontsize;
    		
    		//count frequency
    		Double fontsizeDouble = new Double(fontsize);
    		Integer fs_count = modeMap.get(fontsizeDouble);
    		if(fs_count == null)
    		{
    			modeMap.put(fontsizeDouble, new Integer(1));
    		}
    		else
    		{
    			modeMap.put(fontsizeDouble, new Integer(fs_count.intValue() + 1));
    		}
    		
    	}
    	
    	
    	if(numFirstCarIndent < 5 && numFirstCarIndent > 0)
    	{
    		indentation_avgoffirst4 = indentation_avgoffirst4/(double)numFirstCarIndent; 
    	}
    	...
    	if(!tps.isEmpty())
    	{
    		first_char_pos_y = tps.get(0).getY();
    		avg_pos_y /= (double)tps.size();
    		pos_x_start = tps.get(0).getX();
    		pos_x_end = tps.get(tps.size() - 1).getX();
    		
    	}
    	
    	//calculate avg fontsize
    	fs_avg = fs_avg/((double)numNonSpaceTPs);
    	
    	//calculate variance
    	for(TextPosition tp: tps)
    	{
    		if(tp.getCharacter() == null) continue;
    		fs_variance += Math.sqrt(Math.abs(fs_avg - tp.getHeight()));
    	}
    	fs_variance = fs_variance/((double)numNonSpaceTPs);
    	
    	//calculate mode fontsize
    	Double modeFontsize = null;
    	int modeMaxFreq = 0;
    	for(Double key: modeMap.keySet())
    	{	Integer val = modeMap.get(key);
    		if(val == null) continue;
    		if(val.intValue() > modeMaxFreq)
    		{	modeMaxFreq = val.intValue();
    			modeFontsize = key; 
    		}
    	}
    	
    	if(modeFontsize != null) fs_mode = modeFontsize.doubleValue();
    	
    	//form a result string
    	
    	String fontStyleText = "";
    	for(String ft: fontStyleSet)
    	{
    		fontStyleText += ft+" ";
    	}
    	fontStyleText = fontStyleText.trim().replace(" ", ",");
    	
    	
    	result = "[::line_info_start::]"
    		+ "line="+lineNumber
    		+ "|fs_avg="+fs_avg
    		+ "|fs_mode="+fs_mode
    		+ "|fs_first="+fs_first
    		...
    		+ "|are_all_chars_bold="+are_all_chars_bold
    		+ "|num_words="+num_words
    		+ "|indentation="+indentation
    		+ "|indentation_avgoffirst4="+indentation_avgoffirst4
    		+ "|num_fontstyle_switches="+numFontStyleSwitches
    		+ "|font_styles="+fontStyleText
    		+ "|first_char_pos_y="+first_char_pos_y
    		+ "|avg_pos_y="+avg_pos_y
     ...
     * 
     * Modified by Suppawong to add info to the output like fontsize etc.
     */
        ...
        for( int i = 0; i < charactersByArticle.size(); i++)
        {
            ...
            while( textIter.hasNext() )
            {
                ...
                if( lastPosition != null )
                {
                    ...
                    {	
                    	//Suppawong: add info here
                    	String info = getInfoString(line, ++curLineNumber);
                    	LinkedList<String> l = new LinkedList<String>(normalize(line,isRtlDominant,hasRtl));
                    	l.addFirst(info);
                    	//writeLine(normalize(line,isRtlDominant,hasRtl),isRtlDominant);
                    	writeLine(l,isRtlDominant);
                        line.clear();
