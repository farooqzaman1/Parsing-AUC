 ...
 */
public class LingPipe_PhraseChunker implements Chunker {
    ...
    public LingPipe_PhraseChunker(HmmDecoder posTagger,
    ...
    public static void old_main(String[] args) {
        ...
        LingPipe_PhraseChunker chunker = new LingPipe_PhraseChunker(posTagger,tokenizerFactory);
...
/////////////////////Suppawong's territory////////////////////////
    //default ctor
    public LingPipe_PhraseChunker(boolean defaultInitialization, HmmDecoder _posTagger, TokenizerFactory _tokenizerFactory)
    {	
    	
    	mPosTagger = _posTagger;
        mTokenizerFactory = _tokenizerFactory;
    	if(defaultInitialization) defaultInit();
    }
    
    /*
     * Default initialization using brown corpus
     * */
    FastCache<String,double[]> cache = null;
    HiddenMarkovModel posHmm = null;
    HmmDecoder posTagger = null;
    TokenizerFactory tokenizerFactory = null;
    LingPipe_PhraseChunker chunker = null;
    public void defaultInit()
    {
    	// parse input params
		File hmmFile = null;
        
		hmmFile = new File("lingpipe-4.0.1/demos/models/pos-en-general-brown.HiddenMarkovModel");
		if(!hmmFile.exists())
		{	hmmFile = new File("../lingpipe-4.0.1/demos/models/pos-en-general-brown.HiddenMarkovModel");
		}
        int cacheSize = 50000;
        
         cache = new FastCache<String,double[]>(cacheSize);
     ...
     // construct chunker
         ...
         posTagger = new HmmDecoder(posHmm,null,cache);
         tokenizerFactory= IndoEuropeanTokenizerFactory.INSTANCE;
         chunker= new LingPipe_PhraseChunker(posTagger,tokenizerFactory);
         //chunker.posTagger = posTagger;
         //chunker.tokenizerFactory = tokenizerFactory;
    }
    
    public Vector<String> getNounPhrasesFromSentence(String sentence)
    {	
    	assert(chunker != null);
    	Vector<String> nPhrases = new Vector<String>();
    	//System.out.println("\n" + sentence);
        String[] tokens 
            = tokenizerFactory
            .tokenizer(sentence.toCharArray(),0,sentence.length())
            .tokenize();
        List<String> tokenList = Arrays.asList(tokens);
        Tagging<String> tagging = posTagger.tag(tokenList);
        //for (int j = 0; j < tokenList.size(); ++j)
    ...
    public Vector<String> getVerbPhrasesFromSentence(String sentence)
    {	
    	assert(chunker != null);
    	Vector<String> nPhrases = new Vector<String>();
    	//System.out.println("\n" + sentence);
    ...
    public Vector<String> getNounPhrasesFromFile(String filepath)
    {	Vector<String> sentences = SentenceProducer.getSentnecesFromText(DocumentUtil.readText(filepath));
    	Vector<String> nPhrases = new Vector<String>();
    	for(String sentence:sentences)
    	{	//Util.jout("SENT: "+sentence+"\n");
    		nPhrases.addAll(getNounPhrasesFromSentence(sentence));
    	}
    	return nPhrases;
    }
    
    public Vector<String> getNounPhrasesFromText(String text)
    ...
    public static void main(String args[])
    {
    	...
    	{	LingPipe_PhraseChunker p = new LingPipe_PhraseChunker(true, null, null);
    		Vector<String> n = p.getNounPhrasesFromFile(args[1]);
    		
    		for(int i = 0;i < n.size(); i++)
    		{
    			System.out.println(n.elementAt(i));
    		}
    	}
    }
